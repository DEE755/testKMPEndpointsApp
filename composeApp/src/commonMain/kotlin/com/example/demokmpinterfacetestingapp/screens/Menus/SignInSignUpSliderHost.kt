...
@Composable
fun SignInSignUpSliderHost(closeDrawer: () -> Unit) {
    ...
    Column {
        ...
        CardButton({ closeDrawer.invoke()
            scope.launch { showToast("Not Implemented Yet") } }) {
            Text("< Contact Us")
...
@Composable
fun SliderConnected(closeDrawer: () -> Unit) {
    val viewModel = authViewModel
    val connectionStatus by viewModel.sessionManager.connectionStatus.collectAsState()
    val uiState by viewModel.uiState.collectAsState()
    val scope = rememberCoroutineScope()
